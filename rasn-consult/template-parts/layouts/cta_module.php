...
$btn_style  = '';
$btn_styles = array();
$sanitized_btn_bg   = sanitize_hex_color($btn_bg_color);
$sanitized_btn_text = sanitize_hex_color($btn_text_color);
if ($sanitized_btn_bg) {
    $btn_styles[] = 'background-color: ' . $sanitized_btn_bg . ';';
}
...
if( $cta_border_radius ):
    $section_style .= ' border-radius: ' . esc_attr($cta_border_radius) . 'px; margin: 2rem;';
endif;

?>

<section class="cta-section" <?php echo $section_style; ?> id="<?= get_sub_field('section_id') ?? '' ?>" >
